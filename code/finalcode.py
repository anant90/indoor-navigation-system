...
def interruptSearch(data):
	dumlist = []
	for w in l:
		found = KnuthMorrisPratt(w.name, data)
		if(found == 1 ) :
			dumlist.append(w.nodeID)
	if ( len(dumlist) == 1 ):
		return dumlist[0]
	elif ( len(dumlist) == 0 ) :
		audio.say("No destination containing the name "+data)
		audio.say("This interrupt key is not valid. Press * to continue")
		return -1
	else :
		closest = dumlist[0]
		D,P = Dijkstra(Gr,curNode)
		for t in dumlist :
			if(D[getnodebynum(t)]<D[getnodebynum(closest)]):
				closest = t
		return ( closest ) 
...
def navigate(d):
	if(True):
		...
		for i in range(len(pathlist)-1):
			if (pathlist[i].n == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode1):
						flag = False
						global curNode
						curNode = pathList[i+1]
					if keyboard.pressed(EScancode3):
						bt_sendID("%d"%(pathlist[i+1].nodeID,))
					if keyboard.pressed(EScancode4):
						global interrupt
						interrupt = True
						global interruptDestination
						interruptDestination = interruptSearch("Toilet")
					if keyboard.pressed(EScancode5):
						global interrupt
						interrupt = True
						global interruptDestination
						interruptDestination = interruptSearch("Stair")
					if keyboard.pressed(EScancode6):
						global interrupt
						interrupt = True
						global interruptDestination
						interruptDestination = interruptSearch("Lift")
			flag = True
			if (pathlist[i].ne == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode6):
						...
						interruptDestination = interruptSearch("Lift")
			flag = True	
			if (pathlist[i].e == pathlist[i+1].nodeID and not interrupt):
			...
			if (pathlist[i].se == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode6):
						...
						interruptDestination = interruptSearch("Lift")
			flag  = True
			if (pathlist[i].s == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode6):
						...
						interruptDestination = interruptSearch("Lift")
			flag  = True
			if (pathlist[i].sw == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode6):
						...
						interruptDestination = interruptSearch("Lift")
			flag  = True
			if (pathlist[i].w == pathlist[i+1].nodeID and not interrupt):
				...
				while flag:
					...
					if keyboard.pressed(EScancode6):
						...
						interruptDestination = interruptSearch("Lift")
			flag  = True			
			if (pathlist[i].nw == pathlist[i+1].nodeID and not interrupt):
...
while running :
	if keyboard.pressed(EScancodeStar):
		...
		navigate(data1)
	if (interrupt == True):
		if(interruptDestination != -1):
			temp = getnodebynum(interruptDestination)
			navigate(temp.name)
			global interrupt
			interrupt = False
	e32.ao_yield()
